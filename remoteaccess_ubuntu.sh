...
then
	...
	net=`ip route show | grep src | cut -d " " -f1`
	echo "Please, wait while your network is scanned"
	completescan=`nmap -sP $net | grep "scan report" | cut -d " " -f5`
	clear
	echo "$completescan" > $f
	...
	cat $f | grep -v $HOSTNAME > /tmp/net #On ubuntu nmap also shows your own hostname
	cat /tmp/net > $f
	rm -rf /tmp/net
